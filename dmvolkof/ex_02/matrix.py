...
class Matrix(object):
    ...
    def transpose(self):
        """ Транспонированное представление матрицы 
            ...
            Aij = Aji
        """
        matn,matm = self.getRank()
        resultMatrix = Matrix(matm, matn)
        for i in range(matn):
            for j in range(matm):
                resultMatrix[j][i] = self.array[i][j]
    ...
    def __add__(self, mat):
        ...
        resultMatrix = Matrix(size[0], size[1])
        
        for i in range(self.n):
            for j in range(self.m):
                resultMatrix[i][j] = self.array[i][j]+mat[i][j]
    ...
    def dot(self, mat):
        ...
        returnMatrix = Matrix(self.n, matm)
        for i in range(self.n):
            for j in range(matm):
                sum = 0
                for k in range(self.m):
                    sum = sum + self.array[i][k]*mat[k][j]
